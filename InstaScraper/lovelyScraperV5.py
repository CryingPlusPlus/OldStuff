...
#some friendly messanges
print('\nMoin')
print('hiermit kannst du Bilder aus Instagram srapen... benutze entweder run(directory, n_pics, kp_file) oder manuell falls etwas nicht funktioniert\n')
...
def scraper(name_index, known_pics, directory, n_pics):
	...
	while True:
		...
		try:
			...
			time.sleep(random.uniform(0, 0.5))
			#bricht die Endlosschleife falls kein response kommt
			if error >= 1000:
				break
	...
	print('\nBitte nicht vergessen, die gefundenen Bilder mit setKnownPics() zu speichern :), falls du diese scrape() manuell gecalled hast\n')


#handler, damit man die Befehle in der -i Shell nicht sebst tippen muss...
def run(directory, n_pics, kp_file):
